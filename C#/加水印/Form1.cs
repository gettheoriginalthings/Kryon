...
namespace 加水印 {
    public partial class 加水印 : Form {
        ...
        int 变色计数 = 0;
        TabPage 说明;
        Bitmap 青蛙图 = new Bitmap(128, 128, PixelFormat.Format24bppRgb);
        ...
        public void 画青蛙() {
            string 青蛙 = "";
            Graphics g = Graphics.FromImage(青蛙图);
            g.Clear(SystemColors.Control);
            int r = 3;
            for (int y = 0; y < 21; y++)
                for (int x = 0; x < 34; x++)
                    if (青蛙[y * 34 + x] == '1')
                        g.FillRectangle(new SolidBrush(Color.Green), x * r, y * r + 20, r, r);
            g.Dispose();
        }
        ...
        private void 代码地址linkLabel_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e) {
            Clipboard.SetText("https://github.com/becomequantum/Kryon");
            欢迎使用本软件ToolStripMenuItem.Text = "代码地址已复制到剪贴板!";
        }
    }
}
